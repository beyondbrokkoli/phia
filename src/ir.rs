...
impl IrBackend {
        ...
        while i < self.ir.len() {
            if let Instruction::BeginWhile = self.ir[i] {
                ...
                for j in i + 1..self.ir.len() {
                    match self.ir[j] {
                        Instruction::Less {
                            ...
                            if let Some(Instruction::WhileCondition { cond_reg }) =
                                self.ir.get(j + 1)
                            {
                                if *cond_reg == target {
                                    idx_reg = Some(left);
                ...
                if let (Some(idx), Some(limit)) = (idx_reg, limit_reg) {
                    ...
                    for table in hoists {
                        self.ir
                            .insert(i, Instruction::EnsureCapacity { table, limit });
                        i += 1;
                    }
                }
            }
    ...
    pub fn generate_rust_code(&self) -> String {
        ...
        for r in used_t {
            out.push_str(&format!(
                "    let mut t_r{}: *mut Table = std::ptr::null_mut();\n",
                r
            ));
        }
        ...
        for instr in &self.ir {
            match instr {
                ...
                Instruction::NewTable { target } => {
                    ...
                    out.push_str(&format!(
                        "    t_r{} = &mut *new_table as *mut Table;\n",
                        target
                    ));
