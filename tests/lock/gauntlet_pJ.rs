...
pub fn run_baked() -> Vec<Box<Table>> {
    ...
    t_r40 = &mut *new_table as *mut Table;
    tables.push(new_table);
    let lim = 200;
    if lim > 0 {
        let t = unsafe { &mut *t_r40 };
        if (lim as usize) > t.array.len() {
            t.array.resize(lim as usize, 0);
        }
    }
    len_r40 = unsafe { (*t_r40).array.len() };
    p_r40 = unsafe { (*t_r40).array.as_mut_ptr() };
    i_r40 = 100;
    loop {
        b_r40 = i_r40 < 200;
        if b_r40 {
            ...
            if (k as usize) < len_r40 {
                unsafe {
                    *p_r40.add(k as usize) = i_r41;
                }
            } else {
