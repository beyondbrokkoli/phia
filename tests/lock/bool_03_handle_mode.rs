// target/release/build/phia-*/out/baked_native.rs

use crate::memory::Table;

#[allow(unused_variables, unused_mut, unused_assignments, clippy::eq_op)]
pub fn run_baked() -> Vec<Box<Table>> {
    let mut b_r0 = false;
    let mut t_r1 = 0i64;
    let mut t_r2 = 0i64;
    let mut tables = Vec::<Box<Table>>::with_capacity(128);

    tables.push(Box::new(Table::new_bool()));
    t_r2 = tables.len() as i64;
    let k = 0;
    if k < 0 {
        panic!("Runtime Error: Negative table index");
    }
    let idx = k as usize;
    if t_r2 == 0 {
        panic!("Runtime Error: table is nil");
    }
    let t = match tables.get_mut((t_r2 - 1) as usize) {
        Some(t) => &mut **t,
        None => panic!("Runtime Error: table is nil"),
    };
    if idx >= t.as_bool_mut().len() {
        t.as_bool_mut().resize(idx + 1, false);
    }
    unsafe {
        *t.as_bool_mut().get_unchecked_mut(idx) = true;
    }
    let k = 1;
    if k < 0 {
        panic!("Runtime Error: Negative table index");
    }
    let idx = k as usize;
    if t_r2 == 0 {
        panic!("Runtime Error: table is nil");
    }
    let t = match tables.get_mut((t_r2 - 1) as usize) {
        Some(t) => &mut **t,
        None => panic!("Runtime Error: table is nil"),
    };
    if idx >= t.as_bool_mut().len() {
        t.as_bool_mut().resize(idx + 1, false);
    }
    unsafe {
        *t.as_bool_mut().get_unchecked_mut(idx) = false;
    }
    tables.push(Box::new(Table::new()));
    t_r1 = tables.len() as i64;
    let k = 0;
    if k < 0 {
        panic!("Runtime Error: Negative table index");
    }
    let idx = k as usize;
    if t_r1 == 0 {
        panic!("Runtime Error: table is nil");
    }
    let t = match tables.get_mut((t_r1 - 1) as usize) {
        Some(t) => &mut **t,
        None => panic!("Runtime Error: table is nil"),
    };
    if idx >= t.as_int_mut().len() {
        t.as_int_mut().resize(idx + 1, 0);
    }
    unsafe {
        *t.as_int_mut().get_unchecked_mut(idx) = t_r2;
    }
    let k = 0;
    if k < 0 {
        panic!("Runtime Error: Negative table index");
    }
    let idx = k as usize;
    if t_r1 == 0 {
        panic!("Runtime Error: table is nil");
    }
    let t = match tables.get((t_r1 - 1) as usize) {
        Some(t) => &**t,
        None => panic!("Runtime Error: table is nil"),
    };
    t_r2 = if idx < t.as_int().len() {
        unsafe { *t.as_int().get_unchecked(idx) }
    } else {
        0
    };
    let k = 0;
    if k < 0 {
        panic!("Runtime Error: Negative table index");
    }
    let idx = k as usize;
    if t_r2 == 0 {
        panic!("Runtime Error: table is nil");
    }
    let t = match tables.get((t_r2 - 1) as usize) {
        Some(t) => &**t,
        None => panic!("Runtime Error: table is nil"),
    };
    b_r0 = if idx < t.as_bool().len() {
        unsafe { *t.as_bool().get_unchecked(idx) }
    } else {
        false
    };
    let k = 0;
    if k < 0 {
        panic!("Runtime Error: Negative table index");
    }
    let idx = k as usize;
    if t_r1 == 0 {
        panic!("Runtime Error: table is nil");
    }
    let t = match tables.get((t_r1 - 1) as usize) {
        Some(t) => &**t,
        None => panic!("Runtime Error: table is nil"),
    };
    t_r2 = if idx < t.as_int().len() {
        unsafe { *t.as_int().get_unchecked(idx) }
    } else {
        0
    };
    println!(
        "{}\t{}\t{}",
        "grid",
        b_r0,
        match tables.get((t_r2 - 1) as usize) {
            Some(t) => format!("table#{}(len={})", t_r2, t.as_bool().len()),
            None => "nil".to_string(),
        }
    );
    return tables;
}

pub const STATS: &str = "fast_sets=0;fast_gets=0;dyn_sets=3;dyn_gets=3;hoists=0;hoist_ctx=;consts_i=6;consts_b=2;consts_f=0;consts_s=0";
