...
pub fn run_baked() -> Vec<Box<Table>> {
    ...
    let t = match tables.get_mut((t_r31 - 1) as usize) {
        Some(t) => &mut **t,
        None => panic!("Runtime Error: table is nil"),
    };
    if idx >= t.array.len() {
    ...
    unsafe {
        *t.array.get_unchecked_mut(idx) = t_r31;
    }
    let k = 0;
    if k < 0 {
        panic!("Runtime Error: Negative table index");
    ...
    let t = match tables.get((t_r32 - 1) as usize) {
        Some(t) => &**t,
        None => panic!("Runtime Error: table is nil"),
    };
...
pub const STATS: &str = "fast_sets=1;fast_gets=0;dyn_sets=3;dyn_gets=2;hoists=1;hoist_ctx=0;consts_i=12;consts_b=0;consts_f=0;consts_s=0";
