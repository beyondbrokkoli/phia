...
pub fn run_baked() -> Vec<Box<Table>> {
    ...
    let t = match tables.get((t_r5 - 1) as usize) {
        Some(t) => &**t,
        None => panic!("Runtime Error: table is nil"),
    };
    t_r7 = if idx < t.as_int().len() {
    ...
    unsafe {
        *t.as_int_mut().get_unchecked_mut(idx) = t_r7;
    }
    i_r0 = 0;
    loop {
        b_r2 = i_r0 < 4;
        if b_r2 {
            ...
            unsafe {
                *t.as_int_mut().get_unchecked_mut(idx) = 9;
            }
            let k = 0;
            if k < 0 {
                panic!("Runtime Error: Negative table index");
            ...
            let t = match tables.get((t_r6 - 1) as usize) {
                Some(t) => &**t,
                None => panic!("Runtime Error: table is nil"),
            };
            t_r7 = if idx < t.as_int().len() {
            ...
            len_r7 = t.as_int_mut().len();
            p_r7 = t.as_int_mut().as_mut_ptr();
            i_r1 = 0;
            loop {
                b_r2 = i_r1 < 4;
                if b_r2 {
