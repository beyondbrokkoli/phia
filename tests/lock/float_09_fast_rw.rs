...
pub fn run_baked() -> Vec<Box<Table>> {
    ...
    let mut p_r28: *mut f64 = std::ptr::null_mut();
    ...
    len_r28 = unsafe { (*t_r28).farray.len() };
    p_r28 = unsafe { (*t_r28).farray.as_mut_ptr() };
    i_r24 = 0;
    loop {
        b_r24 = i_r24 < 8;
        if b_r24 {
    ...
    loop {
        ...
        if b_r24 {
            ...
            if (k as usize) < len_r28 {
                f_r27 = unsafe { *p_r28.add(k as usize) };
            } else {
                panic!("optimizer invariant violated: fast-path bounds check failed");
            }
            f_r26 = f_r26 + f_r27;
