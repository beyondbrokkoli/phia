...
pub fn run_baked() -> Vec<Box<Table>> {
    ...
    let t = match tables.get_mut((t_r23 - 1) as usize) {
        Some(t) => &mut **t,
        None => panic!("Runtime Error: table is nil"),
    };
    if idx >= t.array.len() {
    ...
    let t = match tables.get((t_r23 - 1) as usize) {
        Some(t) => &**t,
        None => panic!("Runtime Error: table is nil"),
    };
...
pub const STATS: &str = "fast_sets=1;fast_gets=0;dyn_sets=2;dyn_gets=1;hoists=1;hoist_ctx=0;consts_i=9;consts_b=0;consts_f=0;consts_s=0";
