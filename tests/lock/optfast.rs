...
pub fn run_baked() -> Vec<Box<Table>> {
    ...
    len_r2 = unsafe { (*t_r2).as_int_mut().len() };
    p_r2 = unsafe { (*t_r2).as_int_mut().as_mut_ptr() };
    i_r0 = 0;
    while i_r0 < 100000 {
        let k = i_r0;
        if k < 0 {
...
pub const STATS: &str = "fast_sets=1;fast_gets=0;dyn_sets=0;dyn_gets=0;hoists=1;hoist_ctx=0;consts_i=3;consts_b=0;consts_f=0;consts_s=0";
