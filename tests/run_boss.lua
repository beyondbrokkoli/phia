...
--
-- The listings live in tests/listing.lua (single source of truth, keyed by
-- discipline — lockdown locks from the same table). Four disciplines:
...
-- THE LISTINGS — tests/listing.lua is the single source of truth; the
-- hygiene section below enforces its rules (duplicate/two-list -> FAIL,
-- magic comments must match the list).
local LISTING = dofile("tests/listing.lua")
local POSITIVE = LISTING.positive
local NEGATIVE = LISTING.negative
local PANIC = LISTING.panic
...
-- tests/diff/ empty and git-clean, flagging the dir as deleted until the
