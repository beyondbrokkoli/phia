...
The `race` benchmark is a deterministic, compute-intensive program designed to compare the native code generated by Delaware against an equivalent C++ implementation. It serves as a stress test for Loop-Invariant Code Motion (LICM), register allocation, and memory aliasing.
...
* **Fair Modulo Semantics:** Lua uses floored modulo, while C++ uses truncated division. To level the playing field, the C++ baseline explicitly implements Lua's rules via `lua_mod()`. This ensures both compilers handle the same extended variable "live ranges" and face identical x86_64 register pressure ceilings.
* **Strict Aliasing Advantage:** Delaware perfectly devirtualizes Lua tables into flat structures. Because its Rust backend guarantees that mutable data (`x`, `y`, `z`) never overlaps, LLVM can aggressively interleave the heavy modulo math. C++ (GCC) cannot mathematically prove non-aliasing here, forcing more conservative instruction scheduling.
* **LICM Validation:** Loop-invariant variables (`aa`, `bb`, `cc`, `modulus`) are deliberately placed inside the hot loop to verify Delaware's hoisting capabilities and ensure zero-overhead abstraction.
* **Verification:** A complete pass over the arrays produces a final modulo-reduced `witness` value, preventing dead-code elimination and guaranteeing deterministic correctness between implementations.
